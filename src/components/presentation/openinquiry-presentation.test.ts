...
describe("OpenInquiryPresentation", () => {
  it("closes the policy sequence with a standards proposal and a concrete provocation", () => {
    ...
    expect(html).toContain("Publishers want to protect their IP");
    expect(html).toContain("and avoid disintermediation.");
    ...
    expect(html).toContain("The goal is simple: the publisher sets the terms, the human brings their own agent, and the agent receives only what those terms allow.");
    ...
    expect(html).toContain("OpenInquiry is a proposal built on experimental WebMCP so people can bring their own agents to expert content on the source&#x27;s terms.");
    ...
    expect(html).toContain("Your subscriptions, society memberships, and hospital-library access do not automatically extend to your agent.");
    ...
    expect(html).toContain("Your agent uses your access on the publisher’s terms.");
    expect(html).toContain("The publisher controls what its Site Tools return.<br/>The physician can view the publisher’s content using their own signed-in access.");
    expect(html).toContain("WebMCP connects the reader’s agent to the publisher’s page.<br/>OpenInquiry would let the publisher decide what the agent gets.");
    ...
    expect(html).toContain("a proposed, independently issued agent data-handling credential.");
    expect(html).toContain("The judge controls that simulated state here; WebMCP does not verify it.");
    expect(html).not.toContain("REQUEST RECEIVED");
    expect(html).toContain("This demo maps those conditions to four exact results.");
    expect(html).toContain("not a WebMCP default or universal character ceiling");
    ...
    expect(html).toContain("WebMCP lets the publisher define what its live, signed-in Site Tools do");
    ...
    expect(html).toContain("Your agent works through the live publisher site and receives only the evidence the publisher permits");
  ...
  it("keeps one anchored editable SVG with explicit relationship semantics", () => {
    ...
    expect(html).toContain("AGENT USES USER ACCESS");
    ...
    expect(html).toContain("AGENT USES PHYSICIAN ACCESS");
    ...
    expect(html).toContain("Every result also carries provenance and a canonical source link.");
